...
using Assignment8.Providers;
using Assignment8.Results;
using System.Net;
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            foreach (var roleClaimValue in model.Roles)
            {
                ...
                {
                    canRegister = false;
                    return BadRequest("Role claim is not allowed");
                }
            }
            ...
            {
                string[] u = customClaim.Split(new char[] { '=' });
                if (u.Length == 2)
                {
                    var claimType = u[0].Trim();
                    var claimValue = u[1].Trim();
                    if (m.AppClaimGetByMatch(claimType, claimValue) == null)
                    {
                        return BadRequest("Custom claim is not allowed");
                    }
                }
                else
                {
                    return BadRequest("Custom Claim value is invalid");
                }
            }
            ...
            {
                return BadRequest("Information sent is not valid");
            }
        }
